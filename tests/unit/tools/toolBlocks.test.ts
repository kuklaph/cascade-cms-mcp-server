...
describe("registerToolBlockTool", () => {
  ...
  test("serializes concurrent add updates without dropping rules", async () => {
    ...
    const first = wrapped({
      action: "add",
      rule: { type: "page", id: "page-1", tools: ["cascade_remove"] },
    });
    await firstWriteStarted;
    const second = wrapped({
      action: "add",
      rule: { type: "block", id: "block-1", tools: ["cascade_edit"] },
    });
    ...
    expect(rules).toEqual([
      { type: "page", id: "page-1", tools: ["cascade_remove"] },
...
describe("tool-block checked tool selection", () => {
  test("draft submit is checked while read cache helpers remain exempt", () => {
    expect(shouldCheckToolBlocks("cascade_draft_submit")).toBe(true);
    expect(shouldCheckToolBlocks("cascade_draft_apply_patch")).toBe(true);
    expect(shouldCheckToolBlocks("cascade_asset_get_value")).toBe(false);
    expect(shouldCheckToolBlocks("cascade_read_response")).toBe(false);
  });
});
